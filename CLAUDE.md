...
one with `bin/rails secret`), `finnhub.api_key`, and a section per connector
...
- One Plaid connection is one **Item** (one institution), and `/accounts/get` returns every account behind it — bank *and* brokerage — so a single Item covers both. `SYNCED_ACCOUNT_TYPES` keeps `investment`/`brokerage`/`depository` and drops credit cards and loans, which are negative balances that `Position.portfolio` (`units > 0`) would silently discard anyway. A bank account's cash comes from `balances.current` via `#cash`; a brokerage's comes through `#positions` as a holding in a security of type `"cash"` — counting both would double the account.
